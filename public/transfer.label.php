<?php
declare(strict_types=1);

require_once __DIR__ . '/../src/PdoWorkItemRepository.php';
require_once __DIR__ . '/../src/Http.php';

use Queue\Http;
use Queue\PdoWorkItemRepository as Repo;

Http::commonJsonHeaders();
if (!Http::ensurePost() || !Http::ensureAuth() || !Http::rateLimit('transfer_label', 10)) { return; }

$in = json_decode(file_get_contents('php://input') ?: '[]', true) ?: $_POST;

$tid    = isset($in['transfer_pk']) ? (int)$in['transfer_pk'] : 0;
$carrier = isset($in['carrier']) ? (string)$in['carrier'] : 'MVP';
$plan   = is_array($in['parcel_plan'] ?? null) ? $in['parcel_plan'] : ['parcels' => []];

if ($tid <= 0) { Http::error('bad_request', 'transfer_pk required'); return; }

$idk = isset($in['idempotency_key']) && $in['idempotency_key'] !== ''
    ? (string)$in['idempotency_key']
    : ("tr:$tid:label:" . sha1(json_encode($plan)));

$payload = [
  'transfer_pk' => $tid,
  'carrier'     => $carrier,
  'parcel_plan' => $plan,         // { parcels: [ { weight_g, items:[{item_id|product_id, qty}] } ] }
  'trace_id'    => Http::requestId(),
];

$jobId = Repo::addJob('generate_label', $payload, $idk);
Http::respond(true, ['job_id' => $jobId, 'idempotency_key' => $idk]);
