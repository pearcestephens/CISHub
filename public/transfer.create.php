...
if (!Http::ensurePost() || !Http::ensureAuth() || !Http::rateLimit('transfer_create', 10)) { return; }

$in = json_decode(file_get_contents('php://input') ?: '[]', true) ?: $_POST;

$tid   = isset($in['transfer_pk']) ? (int)$in['transfer_pk'] : 0;
$lines = is_array($in['lines'] ?? null) ? array_values($in['lines']) : [];
$idk   = isset($in['idempotency_key']) && $in['idempotency_key'] !== '' ? (string)$in['idempotency_key'] : "tr:$tid:create";
...
$jobId = Repo::addJob('create_consignment', $payload, $idk);
