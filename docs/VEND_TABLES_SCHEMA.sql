...
CREATE TABLE `vend_categories` (
  ...
  KEY `ix_vc_name` (`name`)
) ENGINE=InnoDB AUTO_INCREMENT=34108 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `vend_inventory_sync` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `outlet_id` varchar(100) NOT NULL,
  `product_id` varchar(100) NOT NULL,
  `vend_id` varchar(100) DEFAULT NULL,
  `old_amount` int(11) NOT NULL DEFAULT 0,
  `new_amount` int(11) NOT NULL DEFAULT 0,
  `sync_timestamp` timestamp NOT NULL DEFAULT current_timestamp(),
  `operation_id` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_sync` (`outlet_id`,`product_id`,`operation_id`),
  KEY `idx_outlet_product` (`outlet_id`,`product_id`),
  KEY `idx_operation` (`operation_id`),
  KEY `idx_sync_timestamp` (`sync_timestamp`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Audit trail for Vend inventory synchronization operations';

CREATE TABLE `vend_oauth_tokens` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  `domain_prefix` varchar(100) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `is_active` tinyint(1) DEFAULT 1,
  PRIMARY KEY (`id`),
  KEY `idx_outlet_id` (`outlet_id`),
  KEY `idx_domain_prefix` (`domain_prefix`),
  KEY `idx_is_active` (`is_active`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Vend OAuth tokens - separate table for security';
...
CREATE TABLE `vend_outlets_closed_notifications` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Unique identifier for each notification about a closed outlet.',
  `outlet_id` varchar(45) NOT NULL COMMENT 'Unique identifier for a specific vape outlet that has been closed.',
  `created_at` timestamp NULL DEFAULT NULL COMMENT 'Timestamp when a notification about an outlet closure was created.',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=211 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Sentence: This table records notifications of closed outlets, which the business uses to track and manage outlet closures for operational and reporting purposes.  \nTags: outlet closure notifications, closed outlet reports, outlet management, closure tracking, operational reports, outlet status updates';
...
) ENGINE=InnoDB AUTO_INCREMENT=110037163 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Sentence: This table records historical changes in product quantities across different outlets, used for tracking inventory levels and analyzing stock movement over time.  \nTags: inventory history, stock movement report, product quantity changes, outlet stock levels, inventory tracking, historical stock analysis, product restocking trends';
...
CREATE TABLE `vend_queue` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Unique identifier for each record in the vend queue for tracking and reference purposes.',
  `request_id` char(16) DEFAULT NULL,
  `idempotency_key` varchar(64) DEFAULT NULL,
  `correlation_id` varchar(64) DEFAULT NULL,
  `vend_data` mediumtext NOT NULL COMMENT 'Stores JSON data detailing product inventory updates for synchronization with the Vend system.',
  `status` int(11) NOT NULL DEFAULT 0 COMMENT 'Indicates whether the data sync with the vendor system is pending (0) or completed (1).',
  `retry_count` int(11) NOT NULL DEFAULT 0 COMMENT 'The number of times a failed attempt to send data to Vend has been retried.',
  `vend_url` varchar(100) NOT NULL COMMENT 'URL for accessing product data from the external Vend/Lightspeed API.',
  ...
  `vend_resource` varchar(64) DEFAULT NULL,
  `entity_type` enum('transfer','po','inventory','product') DEFAULT NULL,
  `entity_pk` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `queue_index` (`status`,`timestamp`),
  KEY `idx_vend_queue_status_id` (`status`,`id`),
  KEY `idx_vend_queue_status_retry` (`status`,`retry_count`),
  KEY `ix_vq_status` (`status`),
  KEY `ix_vq_status_retry` (`status`,`retry_count`),
  KEY `ix_vq_created` (`created_at`),
  KEY `ix_vq_timestamp` (`timestamp`),
  KEY `ix_vq_source_created` (`source`,`created_at`),
  KEY `ix_vq_endpoint_status` (`endpoint`,`status`),
  KEY `idx_status_due` (`status`,`next_attempt_at`),
  KEY `idx_locked` (`status`,`locked_by`,`locked_at`),
  KEY `idx_idempotency` (`idempotency_key`),
  KEY `idx_entity_ref` (`entity_type`,`entity_pk`),
  KEY `idx_resource_status` (`vend_resource`,`status`)
) ENGINE=InnoDB AUTO_INCREMENT=1323947 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Sentence: This table stores records of inventory updates and synchronization attempts with the Vend system, used to ensure accurate stock levels across platforms and troubleshoot synchronization issues.  \nTags: inventory synchronization, Vend API updates, stock level discrepancies, synchronization status, retry attempts, Vend integration issues';
